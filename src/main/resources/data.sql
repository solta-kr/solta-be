...
LOAD DATA LOCAL INFILE '/Users/leejaehoon/Documents/sideprojects/solta/server/src/main/resources/csv/problem.csv'
    ...
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    (id, title, boj_problem_id, tier, level);
...
SELECT FLOOR(300 + RAND() * 1800)                             as solve_time_seconds, -- 5분 ~ 30분
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B1', 'B2', 'B3', 'S1', 'S2', 'G1', 'G2')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
...
SELECT FLOOR(200 + RAND() * 1500)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B1', 'B2', 'B3', 'S1', 'S2', 'G1')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
...
SELECT FLOOR(350 + RAND() * 1900)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B2', 'B3', 'S1', 'S2', 'G1', 'G2')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
...
SELECT FLOOR(400 + RAND() * 2000)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B1', 'B2', 'B3', 'B4', 'S1', 'S2', 'S3', 'G1', 'G2')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
       DATE_SUB(NOW(), INTERVAL 8 + (t.n * 2) DAY)            as solved_time,
...
SELECT FLOOR(500 + RAND() * 2500)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B1', 'B2', 'B3', 'B4', 'B5', 'S1', 'S2', 'S3', 'S4', 'G1', 'G2', 'G3')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
       DATE_SUB(NOW(), INTERVAL 30 + (t.n * 7) DAY)           as solved_time,
...
SELECT FLOOR(600 + RAND() * 3000)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN ('B1', 'B2', 'B3', 'B4', 'B5', 'S1', 'S2', 'S3', 'S4', 'S5', 'G1', 'G2', 'G3', 'G4')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
       DATE_SUB(NOW(), INTERVAL 90 + (t.n * 7) DAY)           as solved_time,
...
SELECT FLOOR(800 + RAND() * 3500)                             as solve_time_seconds,
       ...
       (SELECT id
        FROM problem
        WHERE tier IN
              ('B1', 'B2', 'B3', 'B4', 'B5', 'S1', 'S2', 'S3', 'S4', 'S5', 'G1', 'G2', 'G3', 'G4', 'G5', 'P1', 'P2')
        ORDER BY RAND()
        LIMIT 1)                                              as problem_id,
       DATE_SUB(NOW(), INTERVAL 180 + (t.n * 30) DAY)         as solved_time,
